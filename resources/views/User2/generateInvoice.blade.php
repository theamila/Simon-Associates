  ...
  <span>status 4</span> --}}
    ...
    @else
        ...
        <div class="form-check d-inline-block ms-5">
            <input class="form-check-input" type="checkbox" id="notifyApprover">
            <label class="form-check-label" style="margin-left: -5px;" for="notifyApprover">Notify Approver</label>
        </div>
    @endif
    ...
    @if ($invoice_data->count() > 0)
        @foreach ($invoice_data as $get)
            ...
            <tr class="fw-bold">
                ...
                <td class="fw-bold text-center fs-3 text-success" style="width: 80px;"><i
                        class="material-symbols-outlined">{{ $get->Reimbursables == 1 ? 'check_circle' : '' }}</i>
                </td>
                ...
                <td style="max-width: 170px; width:170px;" class="text-center">
                    ...
                    <a onclick="return confirm('Are you sure you want to delete this invoice Data?')"
                        href="{{ route('deleteInvoiceData', $get->id) }}" class="btn btn-sm btn-danger">
                        <i class="material-symbols-outlined">
                            delete_forever
                        </i>
                    </a>
    ...
    <script>
        document.getElementById('notifyApprover').addEventListener('change', function() {
            ...
            if (this.checked) {
                ...
                sendBtn.href = baseUrl.replace('false', 'true');
            } else {
                // Set 'false' when checkbox is unchecked
                sendBtn.href = baseUrl;
            }
        });
    </script>
